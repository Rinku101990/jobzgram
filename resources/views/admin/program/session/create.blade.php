...
<div class="container-fluid flex-grow-1 container-p-y">
        <h4 class="font-weight-bold py-3 mb-0">Add Program Session</h4>
        <div class="text-muted small mt-0 mb-4 d-block breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="feather icon-home"></i></a></li>
                <li class="breadcrumb-item">Program Session</li>
                <li class="breadcrumb-item active">Add Program Session</li>
            </ol>
        </div>

        <div class="card mb-4">
            <h6 class="card-header bg-dark text-white">Program Session</h6>
            <div class="card-body">

                <form class="form-horizontal form-label-left" action="{{ url('admin/session') }}" method="post" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            <label class="form-label">Program <span class="error">*</span></label>
                            <select class="form-control"  name="program">
                                <option value="">Select Program</option>
                                @foreach($program as $p_val)
                                <option value="{{$p_val->id}}">{{$p_val->title}}</option>
                                @endforeach
                           </select>
                            <div class="clearfix"></div>
                            @error('program')
